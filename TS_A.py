...
for smertnost_variables.outer_month in smertnost_puti.months:
        ### по строкам "всего Церебр. атероск-з" предыдущего года
    for row in smertnost_begin_sql.cur2.execute(smertnost_variables.SELECT + \
                                                ...
                                                " " + '"' + smertnost_variables.sql_TSereb_ater_z[0] + '"' + " " + \
                                                smertnost_variables.OR + \
                                                " " + '"' + smertnost_variables.column_cause_death + '"' + " " + \
                                                smertnost_variables.LIKE + \
                                                " " + '"' + smertnost_variables.sql_TSereb_ater_z[1] + '"' + " " + \
                                                ")" + " " + \
                                                ")"):
        # учитываем по нарастающей предыдущего года: Церебр. атероск-з
        smertnost_variables.TSA_vsego_prev += row[0]
        # внесение сведений за предыдущий год в свод таблицы смертности "Церебр. атероск-з всего"
        smertnost_begin_sql.cur2.execute(smertnost_variables.UPDATE + \
                                         " " + smertnost_variables.table_death_svod + " " + \
                                         smertnost_variables.SET + \
                                         " " + smertnost_variables.outer_month + " " + \
                                         "=" + \
                                         " " + "'" + str(smertnost_variables.TSA_vsego_prev) + "'" + " " + \
                                         smertnost_variables.WHERE + \
    ...
    # по строкам "всего Церебр. атероск-з" текущего года
    for row in smertnost_begin_sql.cur2.execute(smertnost_variables.SELECT + \
                                                ' ' + smertnost_variables.count_row + ' ' + \
        ...
        # учитываем по нарастающей текущего года: Церебр. атероск-з
        smertnost_variables.TSA_vsego += row[0]
        # проверяет есть ли смертность за данный месяц по поводу Церебр. атероск-з
        if select_cur_total[0] != '0':
            temp_for_sql = smertnost_variables.TSA_vsego
        else:
            # либо оставляем нули до конца текущего года по поводу Церебр. атероск-з
            temp_for_sql = smertnost_variables.tire

        # внесение сведений в обзор смертности за текущий год "Церебр. атероск-з всего"
        smertnost_begin_sql.cur2.execute(smertnost_variables.UPDATE + \
                                         " " + smertnost_variables.table_death_svod + " " + \
                                         smertnost_variables.SET + \
                                         " " + smertnost_variables.outer_month + " " + \
                                         ...
                                         " " + "'" + smertnost_variables.TSA + " " + str(
            smertnost_puti.year_current) + " " + smertnost_variables.year_point + "'")
        # разница за обои года по строкам "всего прирост/убыль Церебр. атероск-з"
        Raznost_TSA = smertnost_variables.TSA_vsego - smertnost_variables.TSA_vsego_prev
        # проверяет есть ли смертность за данный месяц по поводу Церебр. атероск-з
        if select_cur_total[0] != '0':
            temp_for_sql = Raznost_TSA
        else:
            # либо оставляем нули до конца текущего года по поводу Церебр. атероск-з
            temp_for_sql = smertnost_variables.tire

        # внесение разности между обоими годами в свод обзора смертности
        # "всего прирост/убыль Церебр. атероск-з"
        smertnost_begin_sql.cur2.execute(smertnost_variables.UPDATE + \
                                         " " + smertnost_variables.table_death_svod + " " + \
                                         smertnost_variables.SET + \
                                         " " + smertnost_variables.outer_month + " " + \
